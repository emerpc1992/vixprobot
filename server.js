...
function loadLicenses() {
  try {
    if (!fs.existsSync(LICENSES_FILE)) return { accounts: {}, messages: [], next_msg_id: 1, resets: {} };
    ...
    return { accounts: {}, messages: [], next_msg_id: 1, resets: {} };
  }
}
...
function applyExpiry(acc) {
  if (acc.active && acc.expires_at) {
    if (Date.now() > new Date(acc.expires_at).getTime()) {
      acc.active = false;
      acc.deactivated_reason = 'auto_30_dias';
    }
  }
  return acc.active;
}
...
app.post('/api/license/register', (req, res) => {
  ...
  if (!data.accounts[email]) {
    ...
    saveLicenses(data);
    log('Nuevo registro de licencia:', email);
  }
...
app.post('/api/admin/accounts/:email/activate', requireAdmin, (req, res) => {
  ...
  saveLicenses(data);
  log('Cuenta activada manualmente:', email);
